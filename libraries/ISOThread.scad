...
module rolson_hex_bolt(dia,hi,fn=6)
// make an ISO bolt with Rolson wrench sizes
//  dia=diameter, 6=M6 etc.
//  hi=length of threaded part of bolt
{
	hhi = rolson_hex_bolt_hi(dia);
	cylinder(r = rolson_hex_bolt_dia(dia)/2,h = hhi, $fn=fn);
...
module rolson_hex_nut_threadless(dia,odx=0,xtralen=0,hi)
// make an ISO nut with Rolson wrench sizes
//  dia=diameter, 6=M6 etc.
{
	hi = rolson_hex_nut_hi(dia);
	difference()
	{
		cylinder(r = (rolson_hex_nut_dia(dia)+odx)/2,h = hi+xtralen, $fn=6);
		translate([0,0,-0.1])	cylinder(r = dia/2, h =hi + 0.2 +xtralen);
	}
//	translate([0,0,0.1])	thread_in(dia,hi-0.2);
}
