...
union(){
   translate([1.9,fyofs,2.3]) rotate([0,-90,0]){
       ...
       //delle
			translate([-9.61-fscr*0.85,61.1,(5.15+14.26)/2]) rotate([90,0,0]) cylinder(r=fscr,h=5,$fn=64);
	 ...
	 difference(){
				translate([3.54,2,0])linear_extrude(height=6.151)outset(d=2,$fn=16)square([sqw,sqh]);
				translate([3.54+0.74,2+2.3,-0.1])linear_extrude(height=6.351)square([fans*1.017,fans*1.027]);
			}
		};
...
translate([-20,-1,13.64]) cube([60,70,10]);
