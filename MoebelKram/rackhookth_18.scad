...
module rackhook(){
  ...
  union(){
	//rail
  cube([raillen,railth,railh]);
  //rest
  translate([0,-restde,0]) cube([raillen,restde,1]);
  //outer plate
  translate([(raillen/2-flapw/2),0,railh]) cube([flapw,flapth,flaph]);
  //spine of hook 
  translate([(raillen/2+flapw/2),0,railh]) cube([spineth,spineth,flaph]);
  //hook
  translate([(raillen/2-flapw/2)+flapw-hookw,flapth+hookgap,railh]) cube([hookw,flapth,flaph]);
  }//union
 //rounder of spine
  translate([(raillen/2+flapw/2),railth+hookgap/2,railh-0.05]) cylinder(d=hookgap,h=flaph+0.1,$fn=32);
  }//difference
}
