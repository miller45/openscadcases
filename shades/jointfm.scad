...
module bracket(cufn=20) {
	difference() {
		difference() {
			cylinder(r=20, h=60, $fn=cufn);
    ...
    translate([+33.89-(mboxw/2),0,-zs/2+12.4])
    {
        union(){
           ...
           translate([-10.4,0,0]){ // color("red")
                ...
                translate([15/2,0,0]) stdbox(1.5,11.5,8,-1,0,1);
            }
        }
    }
...
*translate([0,0,-(1+tph+tph/2-20)-15]) xbox();

*translate([0,0,-(1+tph+tph/2-20)-70]) botlid();
